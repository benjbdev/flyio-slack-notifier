...
- **Deploy-aware capacity suppression**: `deployInProgress` (in `poller.go`) flags a poll as mid-deploy when machines have mixed `image_ref`s OR any machine diverges from the stored baseline. `capacityTracker.observe` consumes this flag — while true, it updates HWM but suppresses degraded/restored emits and resets the healthy streak. Computed *before* `detectDeploy` updates the store, so the final poll of a deploy (when convergence just happened) still reads as "deploying" and doesn't leak a stray "capacity restored". The suppression has a safety timeout (`defaultDeploySafetyTimeout=15m`); past it, normal degraded alerts resume so a wedged deploy stranded at half-capacity still surfaces — silent failure is worse than a noisy one.
